    ...
    try:
        ...
        try:
            with DDGS(timeout=3) as ddgs:
                results = list(ddgs.text(sanitized_query, backend="brave", max_results=sanitized_num))
        except Exception:
            pass
        ...
        if not results:
            try:
                with DDGS(timeout=3) as ddgs:
                    results = list(ddgs.text(sanitized_query, backend="auto", max_results=sanitized_num))
