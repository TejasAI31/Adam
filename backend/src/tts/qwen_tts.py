"""FasterQwen3TTS Model initialization and chime generation."""
...
# Define classes to mock qwen_tts.inference.qwen3_tts_model
AudioLike = Union[
...
class Qwen3TTSModel:
    """
    A HuggingFace-style wrapper for Qwen3 TTS models that bypasses transformers AutoProcessor/AutoModel.
    """

    def __init__(self, model, processor, generate_defaults: Optional[Dict[str, Any]] = None):
        self.model = model
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Directly load Qwen3 TTS model and its processor, bypassing AutoModel and AutoProcessor.
        """
        print(f"[TTS Direct Load] Loading Qwen3TTSForConditionalGeneration directly: {pretrained_model_name_or_path}")
        model = Qwen3TTSForConditionalGeneration.from_pretrained(pretrained_model_name_or_path, **kwargs)
        processor = Qwen3TTSProcessor.from_pretrained(pretrained_model_name_or_path, fix_mistral_regex=True)
        generate_defaults = getattr(model, "generate_config", {})
        ...
        **kwargs,
        ...
        for ins in instructs:
            if ins is None or ins == "":
                ...
                text_id = self._tokenize_texts([self._build_instruct_text(ins)])[0]
            instruct_ids.append(text_id)
...
# MOCK ONLY the specific submodule causing the transformers AutoProcessor import.
# Do NOT mock the root 'qwen_tts' package so that submodules like 'qwen_tts.core' remain importable.
mock_inference_mod = ModuleType("qwen_tts.inference.qwen3_tts_model")
mock_inference_mod.Qwen3TTSModel = Qwen3TTSModel
mock_inference_mod.VoiceClonePromptItem = VoiceClonePromptItem
sys.modules["qwen_tts.inference.qwen3_tts_model"] = mock_inference_mod


# Now import from core models - this triggers package __init__, which safely resolves from the mocked sys.modules cache
from qwen_tts.core.models import Qwen3TTSConfig, Qwen3TTSForConditionalGeneration, Qwen3TTSProcessor

# Now import FasterQwen3TTS. It will resolve its qwen_tts imports using our mock modules!
from faster_qwen3_tts import FasterQwen3TTS
from config.settings import model_cfg, CHIME_PATH
